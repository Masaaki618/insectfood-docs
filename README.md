...
このアプリは、2022年に Ruby on Rails で開発した昆虫食診断サービスのリプレイスです。

当時のアプリではスコアに紐づく昆虫ランクをもとにランダム表示する仕組みでしたが、今回はAIによるパーソナライズされたレコメンドを実現するため、要件を一から壁打ちして大幅にアップデートしました。

主な変更点：
- **技術スタック刷新**：Ruby on Rails → Flutter（フロントエンド）/ Go + Gin（バックエンド）
- **診断ロジック強化**：単純なスコアランク → 3軸スコアをAIに渡してレコメンド
- **AI連携追加**：Claude Haiku 4.5によるパーソナライズコメントの自動生成
